...
static int pm8xxx_vib_set_on(struct pm8xxx_vib_pwm *vib)
{
	...
	VIB_PWM_INFO("%s \n",__func__);
		...
		rc = gpio_direction_output(vib->vdd_gpio, ENABLE_VDD);
		if (rc < 0) {
			VIB_PWM_ERR("%s enable vdd fail",__func__);
			return -EINVAL;
		}
...
static int pm8xxx_vib_set_off(struct pm8xxx_vib_pwm *vib)
{
	...
	VIB_PWM_INFO("%s \n",__func__);
		...
		rc = gpio_direction_output(vib->vdd_gpio, DISABLE_VDD);
		if (rc < 0) {
			VIB_PWM_ERR("%s disable vdd fail",__func__);
			return -EINVAL;
		}
